interface SharedShowcaseChromeOptions {
    ...
    heroClass: string;
    panelClass: string;
    heroHeadClass: string;
    showcaseLabelClass: string;
...
${selector(heroHeadClass)}{display:flex;align-items:center;justify-content:space-between;gap:12px}
${selector(showcaseLabelClass)}{display:inline-flex;align-items:center;min-height:30px;padding:0 12px;border-radius:999px;background:color-mix(in srgb,var(--color-projects) 14%,transparent);color:var(--color-projects);font-size:.74rem;font-weight:900;letter-spacing:.12em;text-transform:uppercase}
${selector(heroCopyClass)} h1{margin:0 0 10px;font-size:clamp(2rem,4vw,3.2rem);line-height:.98;letter-spacing:-.03em;overflow-wrap:anywhere}
${selector(heroCopyClass)} p{max-width:${heroCopyMaxWidth};margin:0;color:var(--color-text-muted);line-height:1.7}
${selector(metaGridClass)}{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}
...
@media (max-width:840px){${selector(metaGridClass)}{grid-template-columns:repeat(2,minmax(0,1fr))}}
