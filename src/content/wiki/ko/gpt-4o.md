...
factCheck:
  ...
  checks:
    - type: source_match
      result: pass
      summary: "원문에서 GPT-4o를 텍스트만이 아니라 이미지나 오디오가 섞인 작업에 붙여도 되는지 문제로 읽어도 되는지 먼저 확인해뒀어."
      items:
    ...
    - type: adversarial
      result: pass
      summary: "헷갈리기 쉬운 해석 포인트는 GPT-4o를 텍스트만이 아니라 이미지나 오디오가 섞인 작업에 붙여도 되는지 기준으로 한 번 더 의심해보고 정리해뒀어."
      items:
...
## 같이 보면 좋은 모델
- [Gemini](/ko/wiki/gemini/) — 비교 대상으로 자주 같이 묶이는 모델 - [ChatGPT](/ko/wiki/chatgpt/) — 멀티모달 생성·해석 흐름을 같이 볼 때 좋아. - [Codex](/ko/wiki/codex/) — 멀티모달 생성·해석 흐름을 같이 볼 때 좋아. - [o3](/ko/wiki/o3/) — 비교 대상으로 자주 같이 묶이는 모델