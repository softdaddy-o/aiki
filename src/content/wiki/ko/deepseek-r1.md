...
factCheck:
  ...
  checks:
    - type: source_match
      ...
      items:
        - "독자가 먼저 갈라 봐야 할 건 DeepSeek R1를 어떤 작업과 운영 조건에 붙일 모델인지."
    ...
    - type: web_cross_check
      ...
      items:
        - "여기서 먼저 갈라 볼 기준은 DeepSeek R1를 고를 때 접근 채널, 가격, 입력 범위 가운데 무엇을 먼저 봐야 하는지."
        - "출처 1 대조: github.com."
        - "출처 2 대조: api-docs.deepseek.com."
    ...
    - type: number_verify
      ...
      items:
        ...
        - "접근 경로 대조: DeepSeek API의 reasoning 채널로 쓰거나, 공개 웨이트를 받아 자체 호스팅하는 두 경로가 있어."
    - type: adversarial
      result: pass
...
## 이 모델로 무엇을 할 수 있나
이 페이지에서 먼저 볼 건 "성능이 높다"보다 "어떤 일을 맡길 모델인가"야. 추론 특화 MoE 모델이야. DeepSeek는 R1이 강화학습 기반 추론 성능을 전면에 내세운 공개 계열이라고 설명해. DeepSeek API의 reasoning 채널로 쓰거나, 공개 웨이트를 받아 자체 호스팅하는 두 경로가 있어. 예를 들어 수학 풀이, 코드 수정, 장문 계획처럼 중간 단계가 긴 작업 후보를 고를 때 차이가 크게 난다. 그래서 실제 제품에 붙일 모델 선택처럼 한 단계씩 풀어야 하는 작업에 맞는지, 아니면 더 가볍고 싼 모델로도 충분한지 가르는 기준이 돼.
## 왜 중요한가
