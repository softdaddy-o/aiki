...
factCheck:
  ...
  checks:
    - type: source_match
      result: pass
      summary: "원문에서 Gemini가 개별 모델 하나가 아니라 어떤 작업군을 묶는 라인업 이름인지 문제로 읽어도 되는지 먼저 맞춰봤다."
      items:
    ...
    - type: web_cross_check
      result: pass
      sources: 2
      summary: "공식 소스 2건을 나란히 놓고 Gemini 아래에서 어떤 버전 페이지를 봐야 하는지 기준으로 설명이 어긋나지 않는지 다시 봤다."
      items:
    ...
    - type: number_verify
      result: pass
      summary: "가격, 접근 경로, 입력 범위처럼 Gemini 아래에서 어떤 버전 페이지를 봐야 하는지를 가를 때 필요한 정보는 따로 떼서 한 번 더 봤다."
      items:
    ...
    - type: adversarial
      result: pass
      summary: "헷갈리기 쉬운 해석 포인트는 Gemini가 개별 모델 하나가 아니라 어떤 작업군을 묶는 라인업 이름인지 기준으로 한 번 더 의심해보고 정리했다."
      items:
...
- **입력/출력 범위**: 멀티모달 지원으로 보면 돼. 다만 입력 전용인지 출력까지 되는지는 API 문서를 함께 봐야 해. 텍스트 전용인지, 이미지까지 같이 읽는지부터 여기서 갈려.
