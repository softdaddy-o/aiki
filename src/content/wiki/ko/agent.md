...
중요한 건 에이전트가 새 모델 이름이 아니라는 점이야. 같은 LLM이라도 검색, 파일 읽기, 코드 실행, [메모리](/ko/wiki/memory/) 저장, 재시도 같은 층을 얹으면 에이전트처럼 동작할 수 있어.
...
또 에이전트는 "생각"보다 "운영" 문제가 더 크게 터질 때가 많아. 그래서 기사나 제품 소개를 볼 때는 [추론](/ko/wiki/inference/) 능력만 보지 말고, 어떤 도구를 쓸 수 있는지와 어디까지 자동으로 행동하는지도 같이 보는 게 맞아.
