...
factCheck:
  ...
  checks:
    - type: source_match
      result: pass
      summary: "원문에서 Mixtral를 텍스트만이 아니라 이미지나 오디오가 섞인 작업에 붙여도 되는지 문제로 읽어도 되는지 먼저 맞춰봤다."
      items:
    ...
    - type: adversarial
      result: pass
      summary: "헷갈리기 쉬운 해석 포인트는 Mixtral를 텍스트만이 아니라 이미지나 오디오가 섞인 작업에 붙여도 되는지 기준으로 한 번 더 의심해보고 정리했다."
      items:
