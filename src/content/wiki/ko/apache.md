...
## 왜 중요한가
이 라이선스가 자주 언급되는 이유는 상업 사용이 가능하다는 말 한 줄로 끝나지 않고, 기업 배포와 오픈 [가중치](/ko/wiki/weight/) 배포에서 법적 경계와 운영 체크리스트를 같이 제공하기 때문이야. 특히 특허 라이선스 조항이 명시돼 있어서, 그냥 짧은 고지 정도만 두는 MIT류와 다르게 '기능은 열어 두되 분쟁 리스크는 어디까지 정리했는가'를 같이 읽게 만들어.
## 주의해서 볼 점
