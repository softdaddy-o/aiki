...
---
## 한 줄 정의
Gemma라는 이름을 새 모델 하나라고 읽으면 자꾸 헷갈려. Google DeepMind가 멀티모달 생성과 이해를 같이 다루는 라인업 쪽 라인업을 설명할 때 앞에 내세우는 간판에 가깝거든. 그래서 기사에서 이 계열명이 보이면 벤치마크보다 어떤 사용 장면을 키우려는지부터 읽는 편이 덜 틀려.
## 이 모델로 무엇을 할 수 있나
...
## 같이 보면 좋은 모델
- [Gemma 3](/ko/wiki/gemma-3/) — Gemma 3와는 오픈 웨이트 여부와 자체 호스팅 난도를 비교하기 쉬워.
