...
factCheck:
  ...
  checks:
    - type: source_match
      result: pass
      summary: "원문에서 Gemma 3를 텍스트만이 아니라 이미지나 오디오가 섞인 작업에 붙여도 되는지 문제로 읽어도 되는지 먼저 확인해뒀어."
      items:
        - "독자가 먼저 갈라 봐야 할 건 Gemma 3를 텍스트만이 아니라 이미지나 오디오가 섞인 작업에 붙여도 되는지."
    ...
    - type: adversarial
      result: pass
      summary: "헷갈리기 쉬운 해석 포인트는 Gemma 3를 텍스트만이 아니라 이미지나 오디오가 섞인 작업에 붙여도 되는지 기준으로 한 번 더 의심해보고 정리해뒀어."
      items:
...
## 같이 보면 좋은 모델
- [Gemma](/ko/wiki/gemma/) — 비교 대상으로 자주 같이 묶이는 모델 - [Gemini](/ko/wiki/gemini/) — 비교 대상으로 자주 같이 묶이는 모델 - [DeepSeek R1](/ko/wiki/deepseek-r1/) — 비교 대상으로 자주 같이 묶이는 모델 - [Llama](/ko/wiki/llama/) — 비교 대상으로 자주 같이 묶이는 모델