...
---
## 한 줄 정의
Perplexity는 질문을 웹에서 찾고 그 결과를 묶어 답으로 정리해 주는 AI 검색 서비스야. 모델 이름이라기보다 검색 엔진과 답변 인터페이스를 합친 제품 이름으로 보는 게 맞아.
## 어떻게 작동하나
질문이 들어오면 관련 웹 문서를 찾고, 그 내용을 모델이 요약하고 정리해서 답으로 보여줘. 핵심은 답변만 던지는 게 아니라 출처 링크를 같이 붙이고, 후속 질문으로 검색 범위를 계속 좁혀 갈 수 있게 만든다는 점이야.
## 왜 중요한가
Perplexity를 이해하면 누가 더 똑똑한 모델이냐보다, 누가 최신 정보를 어떻게 끌어오고 출처를 어떻게 붙여 주느냐가 중요한 제품이 있다는 걸 바로 알 수 있어. 특히 뉴스, 시장, 조사처럼 내부 지식만으로 답하면 금방 낡는 영역에서 이런 차이가 아주 크게 드러나.
## 주의해서 볼 점
Perplexity가 출처를 보여준다고 해서 답 전체가 자동으로 정확해지는 건 아니야. 검색 품질, 문서 선택, 요약 과정이 한 번 더 개입하니까 링크가 있다는 사실과 링크를 제대로 읽었다는 건 따로 판단해야 해.
## 관련 용어
- [chatgpt](/ko/wiki/chatgpt/)는 범용 챗봇 인상이 더 강하다. Perplexity와 같이 보면 대화 제품과 검색 제품의 중심축 차이가 선명해져.
- [claude](/ko/wiki/claude/)는 긴 문서 해석과 글쓰기 보조 쪽에서 자주 비교된다. Perplexity는 검색 연결이 앞에 있고 Claude는 모델 상호작용이 앞에 있다는 차이가 보여.
...
- [grok](/ko/wiki/grok/)는 질문 응답 제품처럼 보이지만 실시간성, 검색, 브랜딩 조합이 다르게 잡혀 있어. 둘을 비교하면 모델 대결보다 제품 설계 차이가 더 잘 보인다.