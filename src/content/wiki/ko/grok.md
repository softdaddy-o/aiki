...
factCheck:
  ...
  checks:
    - type: source_match
      result: pass
      summary: "원문에서 Grok가 개별 모델 하나가 아니라 어떤 작업군을 묶는 라인업 이름인지 문제로 읽어도 되는지 먼저 맞춰봤다."
      items:
    ...
    - type: web_cross_check
      result: pass
      sources: 2
      summary: "공식 소스 2건을 나란히 놓고 Grok 아래에서 어떤 버전 페이지를 봐야 하는지 기준으로 설명이 어긋나지 않는지 다시 봤다."
      items:
    ...
    - type: number_verify
      result: pass
      summary: "가격, 접근 경로, 입력 범위처럼 Grok 아래에서 어떤 버전 페이지를 봐야 하는지를 가를 때 필요한 정보는 따로 떼서 한 번 더 봤다."
      items:
    ...
    - type: adversarial
      result: pass
      summary: "헷갈리기 쉬운 해석 포인트는 Grok가 개별 모델 하나가 아니라 어떤 작업군을 묶는 라인업 이름인지 기준으로 한 번 더 의심해보고 정리했다."
      items:
...
- [Perplexity](/ko/wiki/perplexity/) — 함께 보면 맥락이 더 빨리 잡힌다.