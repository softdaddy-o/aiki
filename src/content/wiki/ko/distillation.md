...
title: "Distillation"
lang: ko
summary: "Distillation는 기계 학습에서 지식 증류 또는 모델 증류는 큰 모델에서 작은 모델로 지식을 전달하는 프로세스입니다."
...
factCheck:
  ...
  checks:
    - type: source_match
      ...
      items:
        - "독자가 먼저 갈라 봐야 할 건 기계 학습에서 지식 증류 또는 모델 증류는 큰 모델에서 작은 모델로 지식을 전달하는 프로세스입니다."
        - "원문을 보면 기계 학습에서 지식 증류 또는 모델 증류는 큰 모델에서 작은 모델로 지식을 전달하는 프로세스입니다."
        - "별칭 대조: knowledge distillation도 같은 대상을 가리키는지 확인했어."
    ...
    - type: web_cross_check
      ...
      items:
        - "여기서 먼저 갈라 볼 기준은 기계 학습에서 지식 증류 또는 모델 증류는 큰 모델에서 작은 모델로 지식을 전달하는 프로세스입니다."
        - "교차 대조: 기계 학습에서 지식 증류 또는 모델 증류는 큰 모델에서 작은 모델로 지식을 전달하는 프로세스입니다."
...
---
## 한 줄 정의
Distillation를 짧게 잡으면 기계 학습에서 지식 증류 또는 모델 증류는 큰 모델에서 작은 모델로 지식을 전달하는 프로세스입니다 쪽이야. 데이터, 파라미터, 압축, 학습 루프를 어떻게 조정해 품질과 비용 균형을 바꾸는지와 연결돼.
## 어떻게 작동하나
기계 학습에서 지식 증류 또는 모델 증류는 큰 모델에서 작은 모델로 지식을 전달하는 프로세스입니다. 데이터, 파라미터, 압축, 학습 루프를 어떻게 조정해 품질과 비용 균형을 바꾸는지와 연결돼. 예를 들어 더 작은 모델에 큰 모델 출력을 학습시키거나 양자화로 운영비를 줄이는 시도가 여기에 들어가.
## 왜 중요한가
...
- [Quantization](/ko/wiki/quantization/) — Quantization와 비교해 보면 학습과 비용 최적화에서 어디가 다른지 읽기 쉬워.
