...
title: "Phi"
lang: ko
summary: "Microsoft의 상위 모델 계열이다. 기사에서 이름만 나오면 하위 버전과 제품 포지션을 함께 확인해야 한다."
...
---
## 한 줄 정의
Phi는 Microsoft가 묶어 부르는 상위 모델 계열이야. 기사에서 이름만 크게 보일 때가 많아서, 먼저 어떤 하위 버전을 가리키는지부터 잡아야 맥락이 덜 꼬인다. 개별 모델 프로필이 필요하면 이 페이지보다 하위 버전 페이지로 내려가는 게 맞다.
## 이 모델로 무엇을 할 수 있나
Phi 같은 계열 페이지에서는 "무슨 일을 잘하나"보다 "어떤 하위 버전으로 갈라지나"를 먼저 보는 편이 좋아. 무료 체험 여부와 유료 플랜 구성은 배포 채널마다 다르다. API, 앱 구독, 팀 플랜을 나눠서 보는 편이 안전하다. 계열 이름만 알아서는 가격이나 컨텍스트를 못 박을 수 없고, 실제 선택은 하위 버전에서 갈린다.
## 스펙을 읽는 법
...
## 왜 중요한가
상위 계열 페이지가 필요한 이유는 뉴스 제목에 버전명보다 계열명만 남는 경우가 많기 때문이야. Microsoft가 라인업을 어떤 축으로 나누는지 먼저 알아두면, 하위 버전 페이지로 내려갔을 때 왜 가격과 성격이 갈리는지도 훨씬 빨리 보인다.
## 같이 보면 좋은 모델
