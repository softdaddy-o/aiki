...
또 이름이 비슷하다고 항상 같은 범위를 뜻하는 것도 아니야. 어떤 서비스는 function calling, 어떤 서비스는 tool use나 tool calling이라고 부르지만, 단순 JSON 출력만 있는 경우는 실제 도구 실행 루프까지 포함하지 않을 수 있어서 구분해서 봐야 해.
## 관련 용어
