...
## 같이 보면 좋은 모델
- [Claude Sonnet](/ko/wiki/claude-sonnet/) — 비교 대상으로 자주 같이 묶이는 모델 - [Claude](/ko/wiki/claude/) — 비교 대상으로 자주 같이 묶이는 모델 - [OpenAI API](/ko/wiki/openai-api/) — 앱 연결과 통합 관점에서 같이 보면 이해가 쉬워. - [Anthropic API](/ko/wiki/anthropic-api/) — 앱 연결과 통합 관점에서 같이 보면 이해가 쉬워.