...
title: "Sora (소라)"
lang: ko
summary: "Sora (소라)는 OpenAI의 영상 생성 모델이자 그 모델을 앞세운 영상 제작 서비스 이름이야. 텍스트나 이미지에서 짧은 영상을 만들고 다듬는 흐름으로 이해하면 기사 문맥이 빨리 잡혀."
  ...
  checks:
    ...
    - type: web_cross_check
      ...
      items:
        ...
        - "앱의 10초·15초 생성, 웹 스토리보드 확장, ChatGPT 구독 연동처럼 공식 페이지에서 직접 확인되는 운영 정보만 골랐어."
      findings:
...
---
## 한 줄 정의
Sora는 OpenAI가 만든 텍스트·이미지 기반 영상 생성 모델이야. 동시에 `sora.com`과 앱으로 이어지는 영상 제작 경험 전체를 가리키기도 해서, 모델명만으로 읽으면 서비스 문맥을 놓치기 쉬워.
## 이 모델로 무엇을 할 수 있나
실무에서는 광고 시안, 콘셉트 영상, 스토리보드 테스트, 소셜 클립 초안 같은 데 먼저 붙어. 현재 OpenAI는 Sora를 웹과 앱으로 운영하고 있고, 도움말 기준으로 앱에서는 10초나 15초 영상 생성과 편집을, 웹 스토리보드에서는 더 긴 생성 흐름을 밀고 있어.
## 왜 중요한가
Sora가 중요한 이유는 OpenAI가 채팅 밖으로 나가 영상 창작 도구까지 제품선을 넓히는 신호이기 때문이야. 그래서 기사에서 Sora를 볼 때는 데모 품질만 보지 말고 ChatGPT 구독과 어떻게 묶이는지, 편집 기능이 어디까지 붙는지를 같이 봐야 해.
## 같이 보면 좋은 모델
- `GPT-4o`: 대화형 멀티모달 쪽 중심이야. Sora와 같이 보면 OpenAI 안에서 실시간 상호작용과 결과물 생성형 비디오가 어떻게 갈리는지 보여.
- `o3`: 분석과 추론 작업을 맡는 모델이야. Sora와 붙여 보면 OpenAI가 생성 도구와 추론 엔진을 따로 운용한다는 점이 선명해져.
- `ChatGPT`: 제품 경험 이름이야. Sora 접근이 ChatGPT 구독과 묶일 때가 많아서 서비스 문맥을 읽는 기준점이 돼.
- `Codex`: 개발 작업 특화 제품선이야. Sora와 나란히 보면 OpenAI가 영역별로 다른 도구 브랜드를 운영한다는 감각이 생겨.