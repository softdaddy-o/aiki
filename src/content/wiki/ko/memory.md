...
factCheck:
  ...
  checks:
    - type: source_match
      ...
      items:
        - >-
          독자 문제 대조: memory를 하드웨어 RAM이나 단순 긴 context와 같은 말로 읽지 않도록 저장-재주입 구조라는
          점부터 고정했어.
        - >-
          LangChain 글에서 short-term memory 맥락을 확인했고, 깨진 Anthropic 링크는 세부 주장 근거로
          쓰지 않았어.
...
## 주의해서 볼 점
Memory는 [long context](/ko/wiki/long-context/)랑 같지 않아. 많이 담는 능력과 잘 저장하고 다시 꺼내는 능력은 다른 문제라서, 오래된 기록의 오염과 개인정보 저장 범위를 같이 봐야 해.
## 관련 용어
- [long-context](/ko/wiki/long-context/): long context는 한 번에 많이 넣는 쪽이야. memory는 나중에 다시 꺼내 쓰는 쪽이라서 둘을 같은 말로 보면 기사 해석이 자꾸 어긋나.
- [tool-use](/ko/wiki/tool-use/): memory는 어떤 도구를 언제 다시 쓸지 결정할 때 자주 같이 붙어. 그래서 둘을 같이 보면 에이전트의 행동 흐름이 더 잘 보여.
- [langchain](/ko/wiki/langchain/): LangChain은 memory 패턴을 코드 단위로 자주 설명하는 프레임워크야. 구현 예시를 볼 때 기준점으로 쓰기 좋아.
- [llamaindex](/ko/wiki/llamaindex/): LlamaIndex는 저장한 정보를 다시 찾아 붙이는 흐름을 볼 때 같이 보면 편해. 특히 검색과 memory의 경계가 어디인지 감 잡기 좋지.
