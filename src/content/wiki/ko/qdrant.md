...
title: "Qdrant"
lang: ko
summary: "Qdrant는 임베딩을 저장만 하는 스토어가 아니라, 필터링과 hybrid search까지 포함해 운영형 검색 계층을 제공하는 오픈소스 벡터 검색 엔진이야. 관리형 서비스 대신 검색 제어권을 더 많이 가져오고 싶을 때 자주 비교되는 선택지다."
readerValue: "벡터를 저장만 하면 되는지, 운영형 검색 계층이 필요한지 먼저 판단하게 해준다."
category: tool
aliases:
  - "Qdrant"
relatedTerms:
...
그래서 Qdrant는 "오픈소스 벡터 DB 하나 더 있다"로 읽으면 안 돼. 운영 제어권을 더 가져오고 싶고, 검색 기능을 세밀하게 튜닝하고 싶을 때 비교해야 하는 도구야. 물론 그만큼 직접 운영 부담도 늘어난다.
## 관련 용어
