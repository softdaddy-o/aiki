...
## 주의해서 볼 점
모든 개선이 training은 아니야. 프롬프트를 잘 쓰는 일, RAG를 붙이는 일, 캐시나 추론 엔진을 최적화하는 일은 보통 가중치를 안 바꾸니까 training과는 다른 층위야. 또 사용자가 서비스에 질문했다고 그 대화가 자동으로 학습에 들어가는 것도 아니어서, 실제로는 수집 정책과 옵트인 여부를 따로 봐야 해.
## 관련 용어
