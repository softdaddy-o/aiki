...
title: "MLflow(엠엘플로우)"
lang: ko
summary: "MLflow는 실험 기록, 모델 버전, 평가 결과, 추적 데이터를 한 흐름으로 묶어 주는 오픈소스 AI 엔지니어링 플랫폼이야. 예전엔 실험 추적 도구 이미지가 강했지만 지금은 LLM과 에이전트 평가, 모니터링 쪽까지 범위를 넓히고 있어."
...
factCheck:
  ...
  checks:
    - type: source_match
      ...
      items:
        - "독자 문제 대조: MLflow를 단순 예전 실험 로그 도구가 아니라, 실험과 모델 운영 기록을 묶는 플랫폼으로 먼저 이해하게 했어."
    ...
    - type: adversarial
      ...
      items:
        ...
        - "팀마다 tracing, registry, evaluation 사용 깊이가 달라서 문맥 없이 이름만 보고 범위를 단정하면 헷갈려."
      findings:
