...
factCheck:
  ...
  checks:
    - type: source_match
      ...
      items:
        - "독자 문제 대조: tokenizer를 독립 모델이나 단순 텍스트 에디터 기능으로 읽지 않도록 입력 전처리 도구라는 점부터 확인했어."
        - "Lexical analysis 설명과 OpenAI tokenizer 도구 맥락을 겹쳐 보고, 토큰화 뒤에 숫자 ID 단계가 따라온다는 축을 본문에 남겼어."
    - type: web_cross_check
      result: pass
      sources: 2
      summary: "일반 토큰화 설명과 LLM 실사용 맥락이 같은 흐름으로 이어지는지 다시 봤어."
      items:
        - "비교 기준: 일반 언어 처리의 tokenization 개념과 LLM 서비스에서 보는 tokenizer 도구가 얼마나 이어지는지 비교했어."
        - "위키 쪽 설명은 개념 정의를 주고 OpenAI 쪽은 실제 토큰 단위 체감 도구를 보여 줘서, 본문을 개념과 실무 영향 둘 다 잡는 방향으로 맞췄어."
    - type: number_verify
      result: skip
