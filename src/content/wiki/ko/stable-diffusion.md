...
---
## 먼저 감 잡기
Stable Diffusion는 Stability AI 라인업 안에서 여러 버전을 묶어 부르는 상위 계열명이야. 기사 제목에는 상위 라인업만 남고 실제 차이는 하위 버전에서 갈리는 경우가 많아서, 먼저 어떤 세부 버전을 가리키는지부터 확인해야 해.
## 뉴스에서 왜 자주 나오나
Stable Diffusion 같은 상위 계열명이 뉴스에서 자주 보이는 이유는 Stability AI 발표가 개별 스냅샷보다 라인업 전체를 먼저 밀기 때문이야. 그래서 기사 제목에는 상위 이름만 남고, 실제 비교 포인트는 하위 버전과 가격표에서 갈리는 경우가 많아. 이 페이지는 그 차이를 놓치지 않게 기준점을 먼저 잡아줘.
## 읽을 때 체크포인트
1. 먼저 Stable Diffusion가 상위 계열명인지, 실제로 바로 붙일 수 있는 개별 버전인지부터 나눠서 읽어야 해.

2. 기사에 Stable Diffusion만 적혀 있으면 하위 버전과 출시 시점을 같이 확인해야 해. 같은 계열이어도 가격, 속도, 입력 범위가 크게 갈릴 수 있어.
