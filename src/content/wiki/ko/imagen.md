...
factCheck:
  ...
  checks:
    - type: source_match
      ...
      items:
        - "독자가 먼저 갈라 봐야 할 건 Imagen가 개별 모델 하나가 아니라 어떤 작업군을 묶는 라인업 이름인지."
        - "모델 이름부터 다시 보면 Imagen."
    ...
    - type: web_cross_check
      ...
      items:
        - "여기서 먼저 갈라 볼 기준은 Imagen 아래에서 어떤 버전 페이지를 봐야 하는지."
...
---
## 한 줄 정의
Imagen라는 이름을 새 모델 하나라고 읽으면 자꾸 헷갈려. Google DeepMind가 멀티모달 생성과 이해를 같이 다루는 라인업 쪽 라인업을 설명할 때 앞에 내세우는 간판에 가깝거든. 그래서 기사에서 이 계열명이 보이면 벤치마크보다 어떤 사용 장면을 키우려는지부터 읽는 편이 덜 틀려.
## 이 모델로 무엇을 할 수 있나
