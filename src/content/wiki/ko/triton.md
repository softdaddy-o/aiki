...
lang: ko
summary: "Triton Inference Server는 여러 프레임워크로 만든 모델을 공통된 서버 형태로 배포하게 해 주는 추론 서버야. 모델을 하나 더 만드는 도구가 아니라, 이미 만든 모델을 운영 환경에서 안정적으로 서빙하는 계층이야."
...
factCheck:
  ...
  checks:
    - type: source_match
      ...
      items:
        - "독자 문제 대조: 이 페이지는 Triton을 모델 이름이 아니라 서빙 계층으로 먼저 이해하게 만들어서 기사 문맥을 바로 잡아 줘."
