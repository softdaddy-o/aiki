...
---
## 한 줄 정의
Model Weights는 학습 과정에서 조정된 숫자 파라미터 집합이야. 쉽게 말해 모델이 무엇을 배웠는지가 실제 값으로 저장된 파일이라고 보면 돼.
## 어떻게 작동하나
모델 구조 코드는 설계도에 가깝고, 웨이트는 그 설계도 안을 채우는 실제 숫자 값이야. 같은 구조라도 어떤 웨이트를 불러오느냐에 따라 답변 스타일, 정확도, 필요한 메모리 구성이 달라져.
## 왜 중요한가
오픈 모델 기사에서 진짜 중요한 차이는 논문 공개 여부보다 웨이트 공개 여부일 때가 많아. 웨이트가 열려야 다른 팀이 직접 추론하거나 미세조정하고, 자기 환경에서 재현성과 배포 가능성을 확보할 수 있어.
## 주의해서 볼 점
