...
[함수 호출](/ko/wiki/function-calling/) 기능을 붙이면 모델이 외부 도구나 API를 호출할 인자까지 제안하게 만들 수도 있어. 그래서 챗봇 하나 만드는 수준을 넘어서 데이터 추출, 업무 자동화, [에이전트](/ko/wiki/agent/) 흐름 같은 앱 구조를 만들기 좋아.
## 왜 중요한가
