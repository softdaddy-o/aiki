...
title: "Gemini API"
lang: ko
summary: "Gemini API 문서 및 API 참조."
...
---
## 한 줄 정의
Gemini API를 짧게 잡으면 문서 및 API 참조 쪽이야. API 키, SDK, 호출 형식, 응답 구조가 실제 통합 난도를 가르는 지점이 돼.
## 실제로 무엇을 하나
API 키, SDK, 호출 형식, 응답 구조가 실제 통합 난도를 가르는 지점이 돼. 예를 들어 프롬프트를 바꿔 보다가 바로 샘플 코드를 내보내 앱에 붙이는 식의 실험이 여기서 자주 일어나. 모델 자체와 같은 층위로 읽으면 도입 범위와 운영 책임을 헷갈리기 쉬워.
## 왜 중요한가
...
- [Gemini](/ko/wiki/gemini/) — Gemini와 함께 보면 Gemini API가 제품, 개념, 모델 가운데 어느 층위인지 비교하기 쉬워.
