...
lang: ko
summary: "Gemini API는 Google 모델을 앱에서 호출할 때 쓰는 개발자용 인터페이스야. 채팅창 이름이라기보다 SDK와 HTTP 요청으로 멀티모달 입력, 구조화 출력, 도구 연결을 붙이는 접점에 가까워."
...
factCheck:
  ...
  checks:
    ...
    - type: web_cross_check
      ...
      items:
        - "비교 기준: Gemini API를 단순 텍스트 생성 API로 볼지, 구조화 출력과 도구 호출까지 포함한 앱 플랫폼 입구로 볼지 맞춰봤어."
        - "Structured Outputs 문서는 JSON Schema 기반 출력을, Function Calling 문서는 외부 도구 연결을, Billing 문서는 무료 티어와 유료 티어 구분을 보여 줬어."
    ...
    - type: number_verify
      ...
      items:
        ...
        - "1M 토큰 컨텍스트는 API 전체 공통이 아니라 일부 모델과 플랜에서 시험 가능한 범위라서, 본문에서는 모델별 차이로 좁혀 적었어."
      findings:
