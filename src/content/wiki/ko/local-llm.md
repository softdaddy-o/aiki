...
보통은 오픈 [웨이트](/ko/wiki/weight/) 모델을 내려받고, [Ollama](/ko/wiki/ollama/)나 [LM Studio](/ko/wiki/lm-studio/), [llama.cpp](/ko/wiki/llama.cpp/) 같은 도구로 실행해. 그러면 인터넷이 느리거나 외부 API를 쓰기 어려운 환경에서도 챗봇, 코딩 보조, 문서 요약 같은 작업을 로컬에서 돌릴 수 있어.

이 방식이 주목받는 이유는 프라이버시와 통제권 때문이야. 입력 데이터를 외부 API로 보내지 않고, 어떤 모델을 쓸지, 어떤 버전으로 돌릴지, 어떤 [양자화](/ko/wiki/quantization/) 설정을 쓸지 직접 정할 수 있으니까.
...
로컬에서 돌린다고 해서 무조건 쉽거나 싸진 않아. 모델 크기, VRAM, [양자화](/ko/wiki/quantization/), 운영체제, 드라이버, 속도 최적화 같은 현실적인 조건이 바로 발목을 잡을 수 있어.
