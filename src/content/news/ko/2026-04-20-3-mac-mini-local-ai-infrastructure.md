...
숫자가 이유를 설명해 줘. 4비트 양자화된 8B 모델 기준으로 초당 18-22 토큰 속도가 나오고, AI 추론 중 소비전력은 30-40W야. 연간 전기료로 계산하면 약 14달러인데, RTX GPU 기반 셋업이 같은 기간 160-210달러 수준이라는 걸 감안하면 비용 차이가 10배 이상이야. 팀 환경이라면 Ollama를 공유 서버로 띄워두고 Open WebUI로 여러 명이 연결하는 방식이 가장 많이 쓰이고 있어.

대부분의 실용적인 작업에서 최적 구성으로 꼽히는 건 64GB 통합 메모리 M4 Pro 모델이야. 14B까지는 여유 있게 돌릴 수 있고, OS와 앱 메모리를 감안해도 공간이 남거든. 다만 14B 이상으로 넘어가면 속도가 초당 10 토큰 이하로 내려가기 때문에 사용 목적에 따라 선택이 달라져.
