...
lang: ko
category: news
summary: "OpenClaw에서 CVE-2026-25253(CVSS 8.8) 등 심각한 보안 취약점이 연달아 발견됐다. 공개 노출된 13.5만 인스턴스 중 63%가 인증 없이 운영 중이며, 관리자 권한 탈취가 가능하다."
...
AI 에이전트 도구 OpenClaw에서 심각한 보안 취약점이 줄줄이 터지고 있다. 3월에만 4일간 9개 CVE가 동시에 공개됐고, 그중 CVE-2026-25253은 CVSS 8.8점으로 게이트웨이 전체 장악이 가능한 수준이다. 악성 링크 클릭 한 번으로 인증 토큰이 유출되면 공격자가 전체 관리자 권한을 가져간다.

근본 원인은 OpenClaw이 [인증 비활성화 상태로 배포](https://www.sangfor.com/blog/cybersecurity/openclaw-ai-agent-security-risks-2026)된다는 점이다. SecurityScorecard STRIKE 팀 조사에 따르면, 공개 노출된 13.5만 개 인스턴스 중 63%가 인증을 설정하지 않은 채 운영 중이었다. WebSocket 핸드셰이크 과정에서 클라이언트가 임의로 `operator.admin` 스코프를 선언할 수 있는 취약점(CVE-2026-33579)도 확인됐다. 일반 사용자가 서버에 "나는 관리자"라고 말하면 그대로 관리자가 되는 셈이다.

OpenClaw를 사용 중이라면 즉시 v2026.3.12 이상으로 업데이트해야 한다. 인증 설정을 반드시 활성화하고, 외부 네트워크 노출 여부를 점검하는 게 급선무다. Anthropic이 Claude 구독에서 OpenClaw을 차단한 것도 이 보안 이슈와 무관하지 않다.
