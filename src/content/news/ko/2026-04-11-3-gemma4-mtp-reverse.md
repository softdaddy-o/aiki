...
MTP는 추론 속도를 높이는 기술이야. 다음 토큰 **1개**만 예측하는 대신, 여러 토큰을 동시에 예측해서 디코딩 단계를 줄여. DeepSeek V3에서 **토큰/초 기준 최대 2배** 향상을 보여줬던 방식인데, Gemma 4에도 같은 구조가 심겨 있었던 거야.

지금은 TFLite 파일 추출까지만 됐고, PyTorch로 변환하는 작업이 커뮤니티에서 진행 중이야. Google이 언제 이걸 공식 지원할지는 미발표 상태야. [역공학 진행 상황](https://huggingface.co/shadowlilac/gemma-4-e4b-mtp-extraction-effort)은 HuggingFace에서 볼 수 있어.
