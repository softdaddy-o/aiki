...
readerValue: "이 페이지는 HyperFrames를 도입해도 되는 팀인지, 그냥 FFmpeg 스크립트나 기존 영상 스택으로 가는 게 나은지 빠르게 가르는 데 초점을 둔다."
...
factCheck:
  ...
  sources:
    ...
    - url: "https://hyperframes.heygen.com/packages/player"
      title: "@hyperframes/player - HyperFrames"
    - url: "https://www.npmjs.com/package/@hyperframes/player"
      title: "@hyperframes/player package"
  ...
  checks:
    ...
    - type: web_cross_check
      ...
      items:
        ...
        - "정적 페이지 임베드는 composition HTML과 `@hyperframes/player` 조합으로 간다. 근거 위치: player package 문서와 npm package 설명."
        - "AIKI showcase component는 `LIVE_RUNTIME_SRC`를 `/hyperframes/showcase-runtime/index.html`로 두고 `hyperframes-player`에 연결한다. 근거 위치: `src/components/projects/showcases/hyperframes/index.tsx`."
        - "정적 runtime 파일에는 `data-composition-id`, `data-duration`, GSAP timeline이 들어 있다. 근거 위치: `public/hyperframes/showcase-runtime/index.html`."
    - type: number_verify
      result: pass
      sources: 3
...
위 Interactive Showcase는 설명용 목업이 아니라 실제 composition을 정적 파일로 배포하고 `@hyperframes/player`로 붙인 결과야. [player 문서](https://hyperframes.heygen.com/packages/player) 기준으로도 개발용 `preview` 서버를 그대로 올리는 게 아니라 composition HTML과 player를 분리해 다뤄.

- `Embedded Player`: 실제 composition 결과가 정상 재생되는지 본다.
...
- `preview`는 개발용이고, 정적 배포는 composition HTML과 player를 따로 다뤄야 해.
