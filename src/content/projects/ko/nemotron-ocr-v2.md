...
modelProfile:
  ...
  multimodalSupport: "문서와 이미지 입력을 다루지만, 범용 생성형 멀티모달 모델이라기보다 OCR과 문서 구조 추출에 더 가깝다."
...
factCheck:
  ...
  checks:
    ...
    - type: web_cross_check
      result: pass
      sources: 2
      summary: "두 출처가 모두 벤더 자료라 독립 비교 검증은 아니야. 그래서 본문도 벤더가 직접 밝힌 설치 조건, 용도, 벤치마크 범위까지만 적었어."
      items:
        - "블로그와 모델 카드는 둘 다 GPU 가속 배포와 문서 구조 인식 용도를 말하지만, 타사 OCR과의 독립 비교 자료까지 주지는 않아."
        - "그래서 본문의 `34.7 pages/s`는 벤더가 공개한 `v2_multilingual` 단일 A100 수치로만 남겼어."
        - "배포 제약 설명도 모델 카드에 직접 적힌 Linux amd64, NVIDIA GPU, Python 3.12, CUDA toolkit 범위로만 묶었어."
    - type: number_verify
      result: pass
      sources: 2
      summary: "본문에 실제로 쓰는 숫자만 다시 맞췄어. 좋아요 수나 variant별 파라미터 수처럼 본문 밖 숫자는 뺐어."
      items:
...
Nemotron OCR v2는 문서와 이미지에서 텍스트를 읽어 내는 OCR 모델이야. 그런데 그냥 글자만 읽는 수준에서 끝나지 않고, 읽은 순서와 문단 관계까지 같이 다루는 쪽에 힘이 들어가 있어. 그래서 스캔 문서, 멀티컬럼 레이아웃, 혼합 언어 문서 같은 입력을 검색 증강 생성, 즉 [RAG](/ko/wiki/rag/) 파이프라인에 넣으려는 팀이 보기 좋다.
...
배포 전제도 가볍지 않다. 모델 카드는 Linux amd64, NVIDIA GPU, CUDA toolkit, Python 3.12를 전제로 두고, 설치 흐름에는 C++ CUDA extension build가 들어간다. 그래서 이 모델은 로컬 CPU 실험보다는 GPU 서버형 PoC나 운영 배포에 더 가깝다.

## 언제 볼 만한가

Nemotron OCR v2는 "GPU 서버에서 OCR을 빠르게 돌려서 [RAG](/ko/wiki/rag/) 입력을 만들고 싶은가"라는 질문에 가깝다.
...
공식 블로그 기준으로 Nemotron OCR v2 multilingual은 단일 A100에서 34.7 pages/s를 제시해. 그리고 블로그가 강조하는 포인트는 두 가지야.
...
PaddleOCR나 더 단순한 OCR 스택은 텍스트 추출이 우선이고, CPU나 상대적으로 가벼운 GPU 환경에서 빨리 붙여야 할 때 유리하다. 언어별 파이프라인을 나눠도 괜찮고, 문단 관계나 읽기 순서까지 구조화해서 넘길 필요가 크지 않다면 이쪽이 실무에서 더 편하다.

반대로 Nemotron OCR v2 multilingual은 다국어 문서를 한 모델로 묶고, 읽기 순서와 레이아웃 관계를 같이 정리해서 [RAG](/ko/wiki/rag/)나 [멀티모달 AI](/ko/wiki/multimodal/) 검색 쪽 후처리 부담을 줄이려는 팀에 맞다. 비교 기준은 "정확도가 누가 더 높냐"보다 "가벼운 추출이 필요한가, 구조까지 담긴 문서 적재가 필요한가"에 가깝다.

## 도입 판단 한 줄 정리

Nemotron OCR v2는 "OCR 정확도 좋은 모델 하나"보다, "GPU 서버에서 다국어 OCR과 문서 구조 추출을 같이 밀어 넣는 문서 적재 모델"에 더 가깝다. RAG, [멀티모달 AI](/ko/wiki/multimodal/) 검색, 에이전트 문서 파이프라인이 중심이면 볼 가치가 있고, 단순 OCR이면 과하다.
