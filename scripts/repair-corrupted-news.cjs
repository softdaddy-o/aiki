...
function buildBody(post, summaryKo, commentKo) {
    ...
    const paragraphs = [
        `${summaryKo} [원문 포스트](${post.postUrl})는 Reddit에서 추천 ${likeCount}개, 댓글 ${replyCount}개를 모으면서 단순 화제성보다 실험 설계 자체로 주목받았어.`,
    ];

    if (facts.length > 0) {
        paragraphs.push(`작성자가 남긴 단서는 ${facts.join(', ')} 쪽이야. 그래서 이 사례는 결과 이미지보다 어떤 제약을 우회했고 무엇이 병목이었는지를 읽어야 의미가 생겨.`);
    }

    paragraphs.push('이런 글을 볼 때 핵심은 "최신 모델이 더 좋다"가 아니라, 제약이 심한 환경에서도 어디까지 동작 범위를 밀어낼 수 있느냐야. 로컬 추론, 엣지 배포, 교육용 실험처럼 성능보다 조건 적합성이 중요한 팀에는 이런 사례가 꽤 직접적인 힌트가 되거든.');

    if (commentKo) {
        paragraphs.push(`커뮤니티 반응도 같은 지점을 짚었어. 상위 댓글은 "${commentKo}"라고 반응했는데, 신기함보다 실제 효용과 재현 가능성을 먼저 묻고 있다는 뜻이야. 그래서 이 글은 데모 감상이 아니라 "어떤 환경에서 이 접근이 먹히는가"를 가르는 사례로 보는 편이 맞아.`);
    } else {
        paragraphs.push('정리하면 이 포스트의 가치는 기술 과시에 있지 않아. 모델 크기, 하드웨어 제약, 툴체인 우회가 한 번에 얽힐 때 로컬 AI의 현실적인 하한선이 어디까지 내려오는지를 보여준다는 점이 더 중요해.');
    }
