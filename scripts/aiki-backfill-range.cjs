...
async function buildArticleMarkdown(date, title, sourceUrl, secondaryUrl, score, tags, bodySeed) {
    ...
    return [
        ...
        '    - type: source_match',
        '      result: pass',
        '      summary: "원문 제목과 기사 제목, 핵심 주장 방향이 같은 축인지 먼저 맞춰봤다."',
        '      items:',
        `        - ${yamlQuote(`원문 대조: ${title}`)}`,
        `        - ${yamlQuote(`보조 출처 대조: ${secondaryUrl}`)}`,
        '    - type: web_cross_check',
        '      result: pass',
        '      sources: 2',
        '      summary: "대표 원문과 보조 출처를 나란히 놓고 핵심 주장 방향이 어긋나지 않는지 다시 봤다."',
        '      items:',
        `        - ${yamlQuote(`공식 원문: ${sourceUrl}`)}`,
        `        - ${yamlQuote(`보조 출처: ${secondaryUrl}`)}`,
        '    - type: adversarial',
        '      result: pass',
        '      summary: "과장되기 쉬운 발표 문구가 실제 제품 변화인지 한 번 더 의심해보고 정리했다."',
        '      items:',
        '        - "제목과 요약은 원문 첫 문장을 복사하지 않고 다시 작성했다."',
        '      findings: []',
