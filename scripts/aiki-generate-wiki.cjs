...
function focusPhrase(entry) {
    ...
    if (tags.includes('retrieval')) return '검색과 외부 지식 연결';
    if (tags.includes('inference') || tags.includes('serving')) return '배포 비용과 추론 처리량';
    if (tags.includes('training')) return '학습 효율과 모델 개선 루프';
    ...
    if (tags.includes('api') || tags.includes('integration')) return '앱 연결과 제품 기능 확장';
    if (tags.includes('safety') || tags.includes('policy')) return '안전성, 제어, 정책 운영';
...
function definitionByCategory(entry) {
    ...
    if (category === 'model') {
        return `${entry.title}는 특정 회사나 연구팀이 내놓은 모델 계열을 가리킨다. 이름 자체보다 중요한 건 이 모델이 ${focus} 쪽에서 어떤 기준점을 만들었는지다. 그래서 발표문을 읽을 때는 벤치마크 숫자만 보지 말고, 어떤 사용 시나리오를 밀고 있는 모델인지 같이 봐야 한다.`;
    }

    if (category === 'tool') {
        return `${entry.title}는 개발자나 팀이 바로 가져다 쓰는 도구에 가깝다. 핵심은 기능 목록보다 이 도구가 ${focus} 쪽의 병목을 얼마나 줄여 주는지다. 뉴스를 읽을 때도 새 기능 소개보다 기존 워크플로에서 어떤 단계가 짧아지는지로 읽으면 훨씬 실무적으로 보인다.`;
    }

    if (category === 'framework') {
        return `${entry.title}는 개별 기능보다 전체 흐름을 짜는 프레임워크다. 보통 ${focus} 같은 문제를 반복 가능한 구조로 묶어 주기 때문에, '지원한다'보다 '어디까지 조립해 주는가'가 더 중요하다. 이 이름이 기사에 나오면 단일 모델 이야기보다 시스템 조합 관점으로 보는 편이 맞다.`;
    }

    if (category === 'technique') {
        return `${entry.title}는 특정 제품명이 아니라 일을 처리하는 방법론에 가깝다. 결국 질문은 하나다. 이 기법이 ${focus} 쪽에서 성능, 비용, 안정성 중 무엇을 바꾸느냐다. 그래서 같은 기법이라도 어떤 모델과 데이터 위에 얹히는지에 따라 뉴스의 무게가 달라진다.`;
    }

    return `${entry.title}는 제품 하나를 뜻하기보다 여러 발표에서 공통으로 쓰이는 개념어다. 이 단어를 잡아 두면 ${focus} 얘기가 나올 때 문장을 훨씬 빨리 해석할 수 있다. 쉽게 말해 기사에 흩어진 표현을 하나의 지도 위에 올려놓게 해 주는 공용 언어라고 보면 된다.`;
}
...
function buildWhyNow(entry, mentionStats, sourceDetails) {
    ...
    if (mentionStats.mentionCount > 0 && mentionStats.firstMentioned) {
        return `${entry.title}는 AIKI 기사에서 ${mentionStats.mentionCount}번 이상 언급됐고, 가장 이른 기록도 ${mentionStats.firstMentioned}까지 올라간다. 그만큼 이 용어는 반짝 유행어라기보다 ${focus} 문제를 설명할 때 계속 재등장하는 기준 단어다. 참고 소스도 ${sourceNames || '공식 문서'} 쪽으로 모여 있어서, 마케팅 문구보다 실제 구현 맥락으로 읽을 여지가 크다.`;
    }

    return `${entry.title}는 아직 기사 수가 많지 않아도 앞으로 자주 붙을 가능성이 높은 단어다. 이유는 간단하다. 새 모델이나 제품이 나와도 결국 독자가 궁금한 건 ${focus} 쪽 변화이기 때문이다. 이런 용어를 먼저 잡아 두면 발표문이 조금 과장돼도 어디를 봐야 하는지 중심을 잃지 않는다.`;
}
...
function buildCheckpoints(entry) {
    ...
    return [
        `1. 먼저 ${entry.title}가 모델 내부 이야기인지, 제품 기능 이름인지, 운영 방식인지부터 구분하면 된다. 같은 단어라도 붙는 위치에 따라 기사 해석이 크게 달라진다.`,
        `2. 다음으로 이 용어가 ${focus} 중 어디를 바꾸는지 봐야 한다. 성능 숫자를 밀어 올리는지, 비용을 줄이는지, 아니면 사용자 경험만 부드럽게 만드는지 나눠서 읽으면 과장을 덜 타게 된다.`,
        `3. 마지막으로 기사에서 ${aliases} 같은 표현이 섞여 나오면 같은 범주인지 하위 변종인지 확인하면 된다. 이름만 다르고 실질은 비슷한 경우가 많아서, 여기서 한 번 걸러 두면 발표 내용을 훨씬 차분하게 정리할 수 있다.`,
    ].join('\n\n');
}
