...
def parse_args():
    ...
    parser.add_argument(
        '--output',
        default=str(Path(__file__).resolve().parents[3] / 'public' / 'data' / 'projects' / 'yfinance.json'),
...
def live_payload():
    if pd is None:
        raise RuntimeError('pandas could not be imported. Reinstall pandas/numpy or run with --demo.')
...
def main():
    ...
    try:
        ...
        print('Install or repair dependencies with: pip install --upgrade --force-reinstall yfinance pandas numpy lxml')
