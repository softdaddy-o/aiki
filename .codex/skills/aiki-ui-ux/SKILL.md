---
name: aiki-ui-ux
description: aiki 프로젝트에서 UI/UX를 만들거나 고칠 때 매번 읽는 가이드. Claude와 Codex 모두 읽는다. 모든 project 쇼케이스는 HyperFrames 를 레퍼런스로 showcase-native 로 만들고, news/wiki 만 standard 포맷을 쓴다. 레이아웃·반응형 규칙과 eval 테스트 포함.
---
...
2. **Project 콘텐츠 구성은 별도 문서가 정한다.** no-prose, panel/card 분해, hero 이후 정보 순서는 `docs/content-guide-projects.md` 를 따른다. 이 스킬은 그 구조를 렌더하는 레이아웃만 맡는다.
...
5. **hero 내용 순서와 panel 정보 위계는 content guide가 정한다.** 이 스킬은 hero shell, meta grid, spacing, responsive behavior 를 맡는다. 프로젝트별 정보 순서는 `docs/content-guide-projects.md` 를 따른다.
6. **포맷 변경 시 `formatVersion` bump 는 필수다 (§3.11).** 이 문서는 레이아웃·템플릿·렌더링 변경만 다룬다. 텍스트 작성 규칙과 해당 버전 관리는 content-guide 문서를 따른다.
...
- 이 스킬은 layout, panel/card grammar, responsive behavior, shared UI, eval만 다룬다.
- summary, readerValue, factCheck 설명문, 카드 카피 같은 텍스트 작성 규칙은 여기서 정의하지 않는다.
- tone 기준은 `docs/tone-guide-common.md` 를 참조한다.
- 텍스트 구조 기준은 `docs/content-guide-common.md` 와 타입별 `docs/content-guide-news.md`, `docs/content-guide-wiki.md`, `docs/content-guide-projects.md` 를 참조한다.
- UI 리뷰에서 copy 이슈를 발견해도, 판단 근거는 content-guide 쪽에 둔다.
...
### 2.7 Project content composition ownership

Project 쇼케이스의 no-prose 규칙, panel set, hero 이후 정보 순서, 카드 헤더 형식은 `docs/content-guide-projects.md` 가 정한다. 이 스킬은 그 콘텐츠 구성을 **어떻게 보이게 렌더할지**만 맡는다.

UI/UX 레벨에서 확인할 것은 아래 세 가지다.
- `usesShowcaseNarrative` 경로로 실제 showcase-native shell 을 타는가
- `Panel id` 와 섹션 네비가 콘텐츠 구조를 제대로 따라가는가
- 카드와 패널이 dense information surface 로 보이되 레이아웃이 무너지지 않는가

### 2.8 Panel/card visual contract

HyperFrames 계열 프로젝트의 콘텐츠 grammar 자체는 `docs/content-guide-projects.md` 를 따른다. 여기서는 그 panel 들이 공통으로 가져야 하는 **시각 계약**만 정의한다.
...
- **2026-04-19**: wiki/news 열 폭 680→720, h1 스케일 up (clamp 26-34), 메타 divider 추가, pill grammar 통일, news site-default cover 억제. before/after 비교 결과 한국어 가독성 개선 + 데스크탑 공기감 확보.
...
### 3.11 포맷 버전 관리 강제 규칙 (CRITICAL)

**이 섹션은 강제다.** 레이아웃·템플릿·렌더링 로직을 바꿀 때마다 반드시 `formatVersion` 을 올려야 한다. 빠뜨리면 나중에 어떤 기사가 어떤 포맷 기준으로 렌더되는지 추적 불가.
...
3. `docs/content-guide-news.md`, `docs/content-guide-wiki.md`, `docs/content-guide-projects.md` 중 해당 타입 guide 의 Changelog 에 한 줄 추가.
4. 이 SKILL.md §3.10 세션 흉터에도 기록.

#### 절대 빠뜨리지 마

- "작은 변경이라 괜찮다" → 없다. 모든 렌더링 변경은 versioned.
- 버전 없이 배포하면 기존 콘텐츠가 새 포맷/기준에서 어떻게 보이는지 확인 불가.
- 새 콘텐츠를 쓰는 Codex 나 다른 에이전트가 잘못된 포맷 버전 참조 → 품질 회귀.
...
1. **타입 결정**:
   ...
   - wiki 항목 → §3 (standard).
   - 그 외는 기본 standard.
2. **Project 인 경우**: HyperFrames 소스 (`src/components/projects/showcases/hyperframes/index.tsx`) 를 열고 `docs/content-guide-projects.md` 의 콘텐츠 구성을 먼저 읽은 뒤, §2.8 visual contract 로 **새 쇼케이스 컴포넌트 설계**.
   - 컨텐츠 분해 기준과 panel set 은 `docs/content-guide-projects.md` 를 따른다.
...
6. **Project-only: content-guide 체크** (`docs/content-guide-projects.md`): `<Content />` 산문이 남지 않았는지, panel set 이 역할대로 분해됐는지.
